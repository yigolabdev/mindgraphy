...
type AcceptanceAction = 'accept' | 'reject'
...
export function MyWeek({ schedule, onAcceptanceChange, onScheduleClick }: MyWeekProps) {
  ...
  const handleAcceptanceClick = (item: MySchedule, action: AcceptanceAction) => {
    setConfirmDialog({
      open: true,
      scheduleId: item.id,
      action,
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        {schedule.length === 0 ? (
          ...
          schedule.map((item) => {
            const acceptanceStatus = (item as any).acceptanceStatus
            ...
            return (
              ...
              >
                <CardHeader className="pb-3">
                  <div className="flex items-start justify-between gap-2">
                    <div className="space-y-1">
                      <h3 className="text-lg font-semibold">{item.groomName} & {item.brideName}</h3>
                ...
                <CardContent className="space-y-3">
                  <ScheduleCardInfo schedule={item} showDday={false} showFullDetails={false} />
      ...
      {/* Confirmation Dialog */}
      <Dialog open={confirmDialog.open} onOpenChange={handleDialogClose}>
        <DialogContent>
          <DialogHeader>
